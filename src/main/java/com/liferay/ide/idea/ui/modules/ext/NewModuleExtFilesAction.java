...
public class NewModuleExtFilesAction extends AnAction implements DumbAware {
	...
	public void update(AnActionEvent event) {
		...
		VirtualFile moduleExtDir = LiferayWorkspaceUtil.getModuleExtFile(event.getProject());

		String extUrl = contextFile.getUrl();

		if (contextFile.equals(moduleExtDir) || (moduleExtDir != null && !extUrl.startsWith(moduleExtDir.getUrl()))) {
			return;
		}
	...
	private class OverrideFilesDialog extends DialogWrapper {
		...
		public void show() {
			...
			application.executeOnPooledThread(this::_setText);

			_overrideFilesPanel.function = () -> _lib;
			_overrideFilesPanel.prepareRefreshButton(_refreshButton, this::_setText);

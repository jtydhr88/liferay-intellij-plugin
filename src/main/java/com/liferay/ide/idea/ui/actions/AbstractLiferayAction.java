...
public abstract class AbstractLiferayAction extends AnAction {
	...
	protected void excludeLiferayWorkspaceBundlesFolder() {
		projectDir.refresh(false, true);

		String basePath = project.getBasePath();

		String bundlesDir = LiferayWorkspaceUtil.getHomeDir(basePath);

		VirtualFile bundlesVirtualFile = projectDir.findChild(bundlesDir);

		if (FileUtil.exist(bundlesVirtualFile)) {
			_excludeFolder(project, bundlesVirtualFile);
		}
	}
	...
	private void _excludeFolder(@NotNull Project project, @NotNull VirtualFile directoryToExclude) {
	...
	private VirtualFile _getContentRoot(@NotNull Module module, @Nullable VirtualFile root) {
		if (root == null) {
			return null;
		}
	...
	private Collection<String> _getOldExcludedFolders(@NotNull Module module, @NotNull VirtualFile directoryToExclude) {
		ModuleRootManager moduleRootManager = ModuleRootManager.getInstance(module);
